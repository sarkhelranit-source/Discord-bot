...
async def pokemon(ctx, *, message):
    ...
    async with request("GET", url) as response:
        data = await response.json()
        emb = nextcord.Embed(
            title=f"{data['name']}", description=f"{data['description']}", color=nextcord.Color.random())
        ...
        emb.set_author(name="Pokemon.com", url="https://www.pokemon.com/us/",
...
async def ip(ctx, *, ipaddr: str = "127.0.0.1"):
    ...
    async with request("GET", url) as response:
        if response.status == 200:
            data = await response.json()
            emb = nextcord.Embed(
                title="IPLOOKUP", color=nextcord.Color.random())
            fields = [
